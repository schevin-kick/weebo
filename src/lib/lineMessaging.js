...
async function sendLineMessage(lineUserId, messages) {
  if (!LINE_CHANNEL_ACCESS_TOKEN) {
    console.warn('LINE_CHANNEL_ACCESS_TOKEN not configured. Skipping message send.');
...
function createBookingCancellationMessage(booking, business, reason = null) {
  const bodyContents = [
    {
      type: 'text',
      text: booking.cancelledBy === 'owner' ? 'Booking Not Approved' : 'Booking Cancelled',
      weight: 'bold',
      size: 'xl',
      color: '#ef4444',
      wrap: true,
...
export async function sendBookingConfirmation(booking, business) {
  ...
  return await sendLineMessage(booking.customer.lineUserId, [message]);
}
